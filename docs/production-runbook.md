...
Должны работать три долгоживущих сервиса: `app`, `postgres`, `redis`, а
одноразовый сервис `migrate` должен завершиться с кодом `0`. Контейнеры с
healthcheck должны перейти в состояние `healthy`. Если миграция завершилась
ошибкой, `app` не запустится; сначала исправьте причину, не обходя этот барьер.
